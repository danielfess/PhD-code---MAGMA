...
//Strategy
//Is beta_5* a square root of beta_2?
...
//beta_2 equation
//Set a to be the coefficients of beta_5* (times a scalar).
a := [T_inv[1,6],T_inv[2,6],T_inv[3,6],T_inv[4,6],T_inv[5,6],T_inv[6,6]];
r := 2;
...
for k := 1 to 6 do
	...
	if k ne r+1 then
		...
		Append(~nonzero_coeff,square_coeff);
	end if;
end for;

//beta_4 equation
b := [T_inv[1,2],T_inv[2,2],T_inv[3,2],T_inv[4,2],T_inv[5,2],T_inv[6,2]];
r := 4;
for k := 1 to 6 do
	square_coeff := 0;
	for i := 1 to 6 do
		for j := 1 to 6 do
			square_coeff +:= mult_coeffs[i,j,k]*b[i]*b[j];
		end for;
	end for;
	if k ne r+1 then
		Append(~square_polys,square_coeff);
...
for k := 1 to 6 do
	...
	for i := 1 to 6 do
		for j := 1 to 6 do
			square_coeff +:= mult_coeffs[i,j,k]*a[i]*b[j];
		end for;
	end for;
	if k ne r+1 then
		Append(~square_polys,square_coeff);
...
print([T_inv[1,3],T_inv[2,3],T_inv[3,3],T_inv[4,3],T_inv[5,3],T_inv[6,3]]);
print(det/(16^3));
