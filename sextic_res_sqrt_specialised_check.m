//Magma code to check a square root of beta_2 and beta_4 (up to scalars), and do a related computation with beta_3.
//The coefficient strings a and b come from other code where we do the primary decomposition.
...
for k := 1 to 6 do
	...
	for i := 1 to 6 do
		for j := 1 to 6 do
			square_coeff2 +:= mult_coeffs[i,j,k]*a[i]*a[j];
			product_coeff3 +:= mult_coeffs[i,j,k]*a[i]*b[j];
			square_coeff4 +:= mult_coeffs[i,j,k]*b[i]*b[j];
		end for;
	end for;
	if k ne 3 then
		Append(~square_polys2,square_coeff2);
	else
		coeff2 := square_coeff2;
	end if;
	if k ne 5 then
		Append(~square_polys4,square_coeff4);
	else
		coeff4 := square_coeff4;
	end if;
	Append(~product_polys3,product_coeff3);
end for;
